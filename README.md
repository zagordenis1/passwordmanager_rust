...
> 📖 **Повна інструкція з покроковими сценаріями:** [`USAGE.md`](USAGE.md)
> — кожна підкоманда, кожен пункт меню, edge-cases експорту/імпорту,
> бекап, скрипти, troubleshooting.
...
3. [Графічний інтерфейс (`pwm-gui`)](#графічний-інтерфейс-pwm-gui)
4. [Інтерактивне меню](#інтерактивне-меню)
5. [Не-інтерактивні підкоманди](#не-інтерактивні-підкоманди)
6. [Auto-lock (автоматичне блокування)](#auto-lock-автоматичне-блокування)
7. [Модель безпеки](#модель-безпеки)
8. [Архітектура](#архітектура)
9. [Тести](#тести)
10. [Реліз-бінарник](#реліз-бінарник)
11. [Міграція з Python-версії](#міграція-з-python-версії)
...
## Графічний інтерфейс (`pwm-gui`)

Окрім CLI-бінарника `pwm` репозиторій містить **необов'язковий**
графічний інтерфейс `pwm-gui` — desktop-додаток на egui/eframe.
Він користується тією самою бібліотекою (`passwordmanagerrs`) і тим
самим форматом БД, тож CLI і GUI можна вільно змішувати: те, що
додано в GUI, читається з `pwm list`, і навпаки.

### Системні залежності GUI на Linux

Окрім тих, що для CLI:
...
На macOS і Windows додаткові пакети не потрібні — eframe лінкується з
системними фреймворками.

### Збірка та запуск

GUI вимикається за замовчуванням, щоб звичайна `cargo build`
залишалась швидкою і не тягла графічні залежності. Увімкніть фічу
`gui` явно:

```bash
# Збірка GUI (разом з бібліотекою + CLI)
cargo build --release --features gui --bin pwm-gui

# Запуск (за замовчуванням використовує ./users.db)
./target/release/pwm-gui

# Або з нестандартним шляхом до БД:
./target/release/pwm-gui /home/user/.local/share/pwm/users.db
```

Інсталяція в `~/.cargo/bin`:
...
- Створення майстер-пароля при першому запуску, розблокування на
  наступних запусках (з підтвердженням пароля + повідомленням про
  невірний пароль).
- Список акаунтів зліва, деталі справа: логін / email / пароль
  (прихований за замовчуванням; кнопки **Показати** і **Копіювати**).
- Додавання акаунтів через модальне вікно з вбудованим генератором
  (довжина 4–4096, класи символів, користувацький набір символів).
- Оновлення пароля та видалення акаунта з підтвердженням.
- Окремий генератор у меню (без додавання акаунта).
- Зміна майстер-пароля з re-encrypt усіх рядків в одній транзакції.
- Експорт у JSON та імпорт із JSON через нативний file picker
  (на Unix експортний файл створюється з режимом `0600`).
- Швидкий пошук по логіну та email.
- Кнопка **🔒 Заблокувати** + auto-lock після `PM_AUTO_LOCK_SECONDS`
  секунд бездіяльності UI (за замовчуванням 300 с; `0` вимикає).
- Копіювання у системний буфер обміну з автоматичним очищенням через
  30 секунд (відлік показано у статус-барі).

### Сумісність із CLI

База одна й та ж: `users.db`. Можна, наприклад, створити майстер-пароль
у GUI, а потім автоматизувати імпорт через `pwm import` у скрипті — і
навпаки. Auto-lock у GUI рахується від UI-активності (миша / клавіші);
у CLI — від інтерактивного вводу. Обидва дотримуються змінної
середовища `PM_AUTO_LOCK_SECONDS`.

### Розмір реліз-бінарника

`pwm-gui` важить ~15–17 МіБ (на відміну від ~7 МіБ у `pwm`) — це ціна
вбудованих шрифтів, GPU-бекенду й нативних діалогів. Якщо вам не
потрібен графічний інтерфейс — просто не вмикайте фічу `gui`,
бінарник `pwm` від цього ніяк не змінюється.
